// Inverser une liste chainee en place : a chaque etape, on retourne le lien du
// noeud courant vers le precedent, puis on avance. Comme le lien `suivant` est
// modifie, il faut sauvegarder le suivant AVANT.
//
// `creer` (construit une liste a partir d'un tableau) et `liberer` sont
// fournies. Complete `inverser` : renvoie la nouvelle tete de la liste.
